...
func (f *Filler) getFunctionByName(field *FieldData) FillerFunc {
	if fc, ok := f.FuncByName[field.Field.Name]; ok == true {
		return fc
	}
...
func (f *Filler) getFunctionByType(field *FieldData) FillerFunc {
	if fc, ok := f.FuncByType[GetTypeHash(field.Field.Type)]; ok == true {
		return fc
	}
...
func (f *Filler) getFunctionByKind(field *FieldData) FillerFunc {
	if fc, ok := f.FuncByKind[field.Field.Type.Kind()]; ok == true {
		return fc
	}
